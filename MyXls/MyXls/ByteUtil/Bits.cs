    ...
    public partial class Bytes
    {
        ...
        public class Bits
        {
            ...
            public uint ToUInt32()
            {
                ...
                if (length > 32)
                    throw new Exception(string.Format("Length {0} must be <= 32", length));
                uint value = 0;
            ...
            public ushort ToUInt16()
            {
                int length = _bits.Length;
                if (length > 16)
                    throw new Exception(string.Format("Length {0} must be <= 16", length));
                ushort value = 0;
            ...
            public ulong ToUInt64()
            {
                int length = _bits.Length;
                if (length > 64)
                    throw new Exception(string.Format("Length {0} must be <= 64", length));
                ushort value = 0;
                for (int i = 0; i < length; i++)
                {
                    if (_bits[i])
