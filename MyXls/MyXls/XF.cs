...
//        private const ushort DEFAULT_LINE_COLOUR_INDEX = 8;
		...
		public string Format
		{
			...
			set
			{
				...
				if (string.Compare(value, _format, StringComparison.InvariantCulture) == 0)
					return;

#if SILVERLIGHT
			    _format = new string(value.ToCharArray());
#else
...
//                    _diagonalLineColorIndex = DEFAULT_LINE_COLOUR_INDEX;
        ...
        private byte[] XF_LINE_COLOUR_STYLE_FILL()
		{
			...
			value += (uint) (Math.Pow(2, 26) * (ushort)_pattern);
			return BitConverter.GetBytes(value);
		}
