		...
		public object Value
		{
			...
			set
			{
				if (value == null)
					...
					_type = TooLargeForInt((int)value) ? CellTypes.Float : CellTypes.Integer;
				else if (value is long)
					_type = TooLargeForInt((long)value) ? CellTypes.Float : CellTypes.Integer;
...
/*        /// <summary>
		...
		private static bool TooLargeForInt(Int64 value)
		{
        ...
        internal void SetValue(byte[] rid, Bytes data)
        {
            if (rid == RID.RK)
                ...
                throw new Exception(string.Format("Unsupported RID {0}", RID.Name(rid)));
        }
