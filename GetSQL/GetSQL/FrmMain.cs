...
WHERE
	...
	AS bit)=0)
	AND tbl.[name] LIKE {0}
ORDER BY [TblName] ASC", StringHelper.QuotedString(this.pTableName.Text + "%"));
			DataSet ds = SqlDAL.ExecuteQuery(strSQL);
	...
	ORDER BY clmns.column_id ASC";
		...
		private string getSelectSQL(DataTable tblColumns, string strTable)
		{
            ...
            //02 获取是否有表前缀
            string strPrefix = string.IsNullOrEmpty(this.txtPrefix.Text.Trim()) ?
                "" : this.txtPrefix.Text.Trim() + ".";
			for (int i = 0; i < tblColumns.Rows.Count; i++)
			{
                //03 判断是否转换小写
                if (this.chkLower.Checked)
                {
                    sb.Append(strPrefix + tblColumns.Rows[i][0].ToString().ToLower());
                }
                else
                {
                    sb.Append(strPrefix + tblColumns.Rows[i][0].ToString());
                }
                if (i < tblColumns.Rows.Count - 1) { sb.Append(", "); }
			}

            sb.Append(Environment.NewLine + "FROM dbo." + strTable + " "
                + this.txtPrefix.Text.Trim()
                + Environment.NewLine
				+ "WHERE 1 = 1 ");
            return sb.ToString();
		...
		private string getInsertSQL(DataTable tblColumns, string strTable)
		{
			...
			foreach (DataRow r in tblColumns.Rows)
			{
                ...
                if (this.chkLower.Checked)
                {
                    strResult += r[0].ToString().ToLower() + ", ";
                }
                else
                {
...
ORDER BY ind.index_id, ind_col.key_ordinal";
			...
			foreach (DataRow r in tblColumns.Rows)
			{
				...
				if(!lstKey.Contains(r[0].ToString()))
				{
                    ...
                    if (this.chkLower.Checked)
                    {
                        strResult += r[0].ToString().ToLower() + " = @" + r[0].ToString().ToLower() + ", ";
                    }
                    else
                    {
